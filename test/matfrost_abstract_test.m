classdef matfrost_abstract_test < matlab.unittest.TestCase
    ...
    methods(TestClassSetup)
        function setup_matfrost(tc, julia_version)
            ...
            configuration_packages_path = fullfile(project_path, "configure_packages.jl");
            cmdline = sprintf( 'julia +%s --project="%s" "%s"', ...
                julia_version, project_path, configuration_packages_path);
            if ispc
            ...
            tc.verifyEqual( resolve_julia_project_status, 0, resolve_julia_project_log);
