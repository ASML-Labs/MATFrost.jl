...
largest_population(p1::SimplePopulationType, p2::SimplePopulationType) =
    ifelse(p1.population > p2.population, p1, p2)
...
sum_nest2_breaking(v::Nest2)::Float64 =
    sum(sum_nest1(e) for e in v.v1) + sum(sum_nest1(e) for e in v.vs; init = 0.0)
...
repeat_string(s::String, num::Int64) = reduce(*, (s for _ = 1:num))
...
function nested_structures_test1(v::Nest3_L1{T}) where {T}
    Nest4_L1{T}(v.v2, v.v1, fill(v, (2, 2, 2)), v.v3, v.v7)
end

function interleave_with_number_and_string(
    vs::Vector{T},
    vnum::Float64,
    vstring::String,
) where {T}
...
for (suf, prim) in (
    ...
    eval(
        :(
            $(Symbol(:nested_structures_test1_scalar_, suf))(v::Nest3_L1{$(prim)}) =
                nested_structures_test1(v)
        ),
    )
    ...
    eval(
        :(
            $(Symbol(:nested_structures_test1_vector_, suf))(v::Nest3_L1{Vector{$(prim)}}) =
                nested_structures_test1(v)
        ),
    )
    ...
    eval(
        :(
            $(Symbol(:interleave_with_number_and_string_, suf))(
                vs::Vector{$(prim)},
                vnum::Float64,
                vstring::String,
            ) = interleave_with_number_and_string(vs, vnum, vstring)
        ),
    )

    eval(
        :(
            $(Symbol(:ifelse_, suf))(b::Bool, v::$(prim), vstrings::Vector{String}) =
                ifelse(b, v, vstrings)
        ),
    )
