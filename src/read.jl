...
function read_matfrostarray_header!(io::IO)::MATFrostArrayHeader
    type = read(io, Int32)
    ndims = read(io, Int64)
    dims = Int64[read(io, Int64) for _ = 1:ndims]
    nel = prod(dims; init = 1)
    MATFrostArrayHeader(type, dims, nel)
end

@noinline function read_matfrostarray_primitive!(
    io::IO,
    header::MATFrostArrayHeader,
    ::Type{T},
)::MATFrostArrayPrimitive{T} where {T<:Number}
    values = Vector{T}(undef, header.nel)
    read_vector(io, values)
    MATFrostArrayPrimitive{T}(header.dims, values)
end

@noinline function read_matfrostarray_string!(
    io::IO,
    header::MATFrostArrayHeader,
)::MATFrostArrayString
    values = String[read_string!(io) for _ = 1:header.nel]
    MATFrostArrayString(header.dims, values)
end

@noinline function read_matfrostarray_struct!(
    io::IO,
    header::MATFrostArrayHeader,
)::MATFrostArrayStruct
    nfields = read(io, Int64)
    fns = Symbol[Symbol(read_string!(io)) for _ = 1:nfields]

    values = MATFrostArrayAbstract[read_matfrostarray!(io) for _ = 1:(nfields*header.nel)]

    MATFrostArrayStruct(header.dims, fns, values)
end

@noinline function read_matfrostarray_cell!(
    io::IO,
    header::MATFrostArrayHeader,
)::MATFrostArrayCell
    values = MATFrostArrayAbstract[read_matfrostarray!(io) for _ = 1:header.nel]
